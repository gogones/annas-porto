...
export default function BlogPost({data, className}) {
  const {title, date, url} = data;
  const language = useContext(LanguageContext);
  const formattedDate = new Intl.DateTimeFormat(
    language.value === 'en' ? 'en-US' : 'id-ID',
    {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
  ...
  return (
    <article
      className={classNames(
        className,
        'focus-within:ring-2 ring-indigo-500 ring-offset-8 rounded-lg'
      )}>
      <a
        href={url}
        className="relative inline-block rounded-lg group focus:outline-none">
